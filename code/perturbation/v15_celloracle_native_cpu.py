#!/usr/bin/env python3
"""Run state-specific native CellOracle TF knockout simulations on donor means."""
...
OUTPUT = ROOT / "05_GRN/v15/celloracle_native"
BASE_GRN = (
    ROOT
...
def run_state(state_slug: str, state_index: int) -> dict[str, str]:
    ...
    with log_path.open("w") as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            targets_table = pd.read_csv(INPUT / "perturbation_targets.tsv", sep="\t")
            targets = targets_table.loc[
                (targets_table["state_slug"] == state_slug)
                & (targets_table["method"] == "CellOracle"),
                "target",
            ].tolist()
            fra = pd.read_csv(
...
def main() -> None:
    ...
    candidates.insert(0, "celloracle_rank_v15", np.arange(1, len(candidates) + 1))
    candidates.to_csv(
        ROOT / "05_GRN/v15/celloracle_native_ko.tsv", sep="\t", index=False
    )
    report = [
        "# Native CellOracle v15",
        "",
